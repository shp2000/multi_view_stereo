...
def compute_rectification_R(T_ji):
    ...
    r3 = np.cross(r1,r2)
    R_irect = np.array([[r1[0], r1[1], r1[2]],
                [r2[0], r2[1], r2[2]],
                [r3[0], r3[1], r3[2]]])
...
def ssd_kernel(src, dst):
    ...
    src = src[:,None,:,:]
    dst = dst[None,:,:,:]
    ssd = np.sum(np.sum((src-dst)**2, axis=2), axis=2)
...
def sad_kernel(src, dst):
    ...
    sad = np.sum(np.sum(np.abs(src-dst), axis=2), axis=2)
...
def zncc_kernel(src, dst):
    ...
    assert src.ndim == 3 and dst.ndim == 3
    assert src.shape[1:] == dst.shape[1:]
    srcm = np.mean(src, axis = 1)
    dstm = np.mean(dst, axis = 1)
    srcsd = np.std(src, axis = 1)
    dstsd = np.std(dst, axis = 1)
    
    zncc= np.zeros((src.shape[0], dst.shape[0], src.shape[2]))
    for i in range(src.shape[2]):
        for j in range(src.shape[0]):
            for k in range(dst.shape[0]):
                zncc[j][k][i] = np.sum(np.multiply(src[j,:,i]-srcm[j][i], dst[k,:,i]-dstm[k][i]))/(np.multiply(srcsd[j][i], dstsd[k][i])+EPS)
...
def image2patch(image, k_size):
    ...
    image = np.pad(image, ((k_size//2, k_size//2), (k_size//2, k_size//2), (0,0) ), 'constant')
    
    patch_buffer = np.zeros((H, W, k_size**2, 3))
    for i in range(H):
        for j in range(W):
            patch_buffer[i,j,:,0] = image[k_size//2+i-k_size//2:k_size//2+i+k_size//2+1, k_size//2+j-k_size//2:k_size//2+j+k_size//2+1, 0].flatten()
            patch_buffer[i,j,:,1] = image[k_size//2+i-k_size//2:k_size//2+i+k_size//2+1, k_size//2+j-k_size//2:k_size//2+j+k_size//2+1, 1].flatten()
            patch_buffer[i,j,:,2] = image[k_size//2+i-k_size//2:k_size//2+i+k_size//2+1, k_size//2+j-k_size//2:k_size//2+j+k_size//2+1, 2].flatten()
...
def compute_disparity_map(
    ...
    ip = image2patch(rgb_i.astype(float) / 255.0, k_size)  
    jp = image2patch(rgb_j.astype(float) / 255.0, k_size)  
    vi = np.arange(H)
    vj = np.arange(H)
    disp = vi[:, None] - vj[None, :] + d0
    
    # if(disp>0.0):
    #     valdisp = disp
    valdisp = disp > 0
    
    for i in range(rgb_i.shape[1]):
        
        val = kernel_func(ip[:,i], jp[:,i])  
        val[~valdisp] = val.max() + 1.0
        vib = np.argmin(val, axis=1)
        vjb = np.argmin(val[:,vib.flatten()], axis=0)
        
        #if(vjb == vi):
        cons = vjb ==vi
        disp_map[:,i] = np.take_along_axis(disp, np.expand_dims(vib, axis=1), axis=1).flatten()
        lr_consistency_mask[cons,i] = 1.0
